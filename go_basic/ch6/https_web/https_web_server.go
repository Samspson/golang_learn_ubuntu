...
func YourListenAndServerTLS(addr string, certFile string, keyFile string, handler http.Handler) error {
	...
	config.Certificates[0], err = YourLocalX509KeyPair(certFile, keyFile)
	if err != nil {
		return err
	}

	conn, errs := net.Listen("tcp", addr)
	if errs != nil {
		return errs
	}
...
func YourLocalX509KeyPair(certFile string, keyFile string) (cert tls.Certificate, err error) {
...
func main() {
	...
	YourListenAndServerTLS(fmt.Sprintf(":%d", SERVER_PORT), "rui.crt", "rui.key", nil)
}
