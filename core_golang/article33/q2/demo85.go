...
func main() {
	...
	fmt.Printf("[ About 'ReadSlice' method ]\n\n")
	// 示例3
	...
	fmt.Printf("[ About 'ReadBytes' method ]\n\n")
	// 示例5
	fmt.Println("Reset the basic reader ...")
	basicReader.Reset(comment)
	size = 200
	...
	delimiter = byte('[')
	fmt.Printf("Read slice with delimiter %q...\n", delimiter)
	line, err = reader3.ReadSlice(delimiter)
	if err != nil {
		fmt.Printf("error: %v\n", err)
	}
	fmt.Printf("Read contents(%d): %q\n", len(line), line)
...
func showContentsLeak(comment string) {
	...
	size := len(comment)
	fmt.Printf("New a buffered reader with size %d ...\n", size)
	read4 := bufio.NewReaderSize(basicReader, size)
	fmt.Println()

	peekNum := 7
	fmt.Printf("Peek %d bytes ...\n", peekNum)
	bytes, err := read4.Peek(peekNum)
	if err != nil {
		fmt.Printf("error: %v\n", err)
	}
	fmt.Printf("Peeked contents(%d): %q\n", len(bytes), bytes)
	...
	fmt.Printf("The all of the contents in the buffered:\n%q\n", bytes)
	fmt.Println()
	...
	// 示例7
	delimiter := byte(',')
	fmt.Printf("Rea slice with delimiter %q ...\n", delimiter)
	line, err := read4.ReadSlice(delimiter)
	if err != nil {
		fmt.Printf("error: %v\n", err)
	}
	fmt.Printf("Read contents(%d): %q\n", len(line), line)
